...
# in this class, the obj and goal distribution will be the same as PandaPush-v2
# The friction will be smaller, the same as PandaSlide-v2
# The obj will be a box, but not a cylinder as the PandaSlide-v1, v2

class Push_fric_range(Task):
    def __init__(
        self,
        sim,
        reward_type="sparse",
    ...
    def _create_scene(self) -> None:
        ...
        self.sim.create_box(
            ...
            lateral_friction=np.random.uniform(low=0.09, high=1.01),  # 0.1 for push3
            # lateral_friction=0.1,  # 0.1 for push3
        )
        self.sim.create_box(
            body_name="target",
            half_extents=np.ones(3) * self.object_size / 2,
