...
# pushing front still
# the goal is in the front, object at the goal
# succeed at beginning, no action needed
class PushRightStill(Task):
    def __init__(
        self,
        sim,
        reward_type="sparse",
        distance_threshold=0.05,
        goal_xy_range=0.00,  # originally 0.3  one direction 0.05

        # temporary for one direction
        goal_x_offset=0.0,
        goal_y_offset=-0.125,
        # temporary for one direction
        obj_xy_range=0.00,
        ...
        self.obj_range_low = np.array([-obj_xy_range / 2 + obj_x_offset, -obj_xy_range / 2 + obj_y_offset, 0])
        self.obj_range_high = np.array([obj_xy_range / 2 + obj_x_offset, obj_xy_range / 2 + obj_y_offset, 0])
    ...
    def _create_scene(self) -> None:
        ...
        self.sim.create_table(length=1.4, width=0.7, height=0.4, x_offset=-0.1)  # original length = 1.4, x_offset = -0.3
        self.sim.create_box(
            body_name="object",
