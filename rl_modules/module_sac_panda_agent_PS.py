...
class module_sac_panda_agent_PS:
            ...
            if MPI.COMM_WORLD.Get_rank() == 0:
                ...
                if epoch % 10 == 0 and self.args.save_data is True:
                    np.save(self.data_path + "/task_hidden_dim{}-interface_dim{}-robot_hidden_dim{}-{}_control-{}_cycles-seed{}-".format(
                        ...
                        self.args.control_type, self.args.n_cycles, self.args.seed) + 'module_sac_real_mixpush2push3_norm_sr', success_rate_record)
                    np.save(self.data_path + "/task_hidden_dim{}-interface_dim{}-robot_hidden_dim{}-{}_control-{}_cycles-seed{}-".format(
                        self.args.task_hidden_dim, self.args.interface_dim, self.args.robot_hidden_dim,
                        self.args.control_type, self.args.n_cycles, self.args.seed) + 'module_sac_real_mixpush2push3_norm_rd', reward_record)
                ...
                if epoch % 10 == 0 and self.args.save_model is True:
                    torch.save(
                        ...
                        self.model_path + "/task_hidden_dim{}-interface_dim{}-robot_hidden_dim{}-{}_control-seed{}-".format(
                            self.args.task_hidden_dim, self.args.interface_dim, self.args.robot_hidden_dim,
                            self.args.control_type, self.args.seed) + 'module_sac_real_mixpush2push3_norm_full_model.pt', _use_new_zipfile_serialization=False)
            ...
            if self.args.save_data is True:
                np.save(
                    self.data_path + "/task_hidden_dim{}-interface_dim{}-robot_hidden_dim{}-{}_control-{}_cycles-seed{}-".format(
                        self.args.task_hidden_dim, self.args.interface_dim, self.args.robot_hidden_dim,
                        self.args.control_type, self.args.n_cycles, self.args.seed) + 'module_sac_real_mixpush2push3_norm_sr',
                    success_rate_record)
                np.save(self.data_path + "/task_hidden_dim{}-interface_dim{}-robot_hidden_dim{}-{}_control-{}_cycles-seed{}-".format(
                    self.args.task_hidden_dim, self.args.interface_dim, self.args.robot_hidden_dim,
                    self.args.control_type, self.args.n_cycles, self.args.seed) + 'module_sac_real_mixpush2push3_norm_rd', reward_record)
    ...
    def permutation_visualize(self, task_path, robot_path):
        ...
        for _ in range(test_num):
            ...
            obs = observation['observation']
            g = observation['desired_goal']
            if distance(obs[self.ee_dim: (self.ee_dim+3)], g) < distance_threshold:
                ini_success_num += 1
                ini_success_flag = 1
            finger_width = obs[6] if (self.args.env_name == "PandaPickAndPlace-v2"
                                      or self.args.env_name == "PandaPush-v4"
            ...
            for t in range(self.env_params['max_timesteps']):
                ee_pos = obs[:3]
                obj_pos = obs[self.ee_dim: (self.ee_dim+3)]
