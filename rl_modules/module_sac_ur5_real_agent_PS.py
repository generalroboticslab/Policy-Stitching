...
This is for few-shot learning (fine-tuning) on real robot
"""


class module_sac_ur5_real_agent_PS:
    def __init__(self, num_task_inputs, num_robot_inputs, args, env, env_params, ta_model_path, ro_model_path):
        ...
        w_star = list(self.actor_network.parameters())
        self.w_star = np.array(self.flat_net([ws.detach().cpu().numpy().tolist() for ws in w_star]))
        ...
        for epoch in range(self.args.n_epochs):
            for _ in range(self.args.n_cycles):
                ...
                for _ in range(self.args.num_rollouts_per_mpi):
                    ...
                    observation = self.env.reset_real2()
                    obs = observation['observation']
                    ag = observation['achieved_goal']
                    g = observation['desired_goal']
                    joins = observation['joint_pos']
                    # start to collect samples
                    success_flag = 0  # if success, don't move robot arm any more
                    # w_epoch = list(self.actor_network.parameters())
                    # w_epoch = np.array(self.flat_net([we.detach().cpu().numpy().tolist() for we in w_epoch]))
                    # net_change = np.linalg.norm(w_epoch - self.w_star)
                    # print('net parameters change: {}'.format(net_change))
                    for t in range(self.env_params['max_timesteps']):
                        ...
                        # check collision with the object, if collide, don't take this action
                        _, _, _, _ = self.env.step(action, control_method=self.args.control_type)  # step in sim first
                        ee_pos = p.getLinkState(self.env.robot.id, self.env.robot.eef_id)[0]  # end effector of the arm without a gripper, a tuple type
                        gripper_tip_pos = np.array(ee_pos) - [0, 0, 0.14]  # gripper is approximately 0.14 in length
                        # only move the arm if it doesn't hit the cube from top, doesn't hit table, and hasn't succeeded
                        if self.env.distance(gripper_tip_pos, ag) > 0.01 and success_flag == 0 and gripper_tip_pos[2] > 0.02:
                            # print("action:")
                            # print(action)
                            observation_new, _, _, info = self.env.step_real2(action, control_method=self.args.control_type)
                            obs_new = observation_new['observation']
                            ag_new = observation_new['achieved_goal']
                            joins_new = observation_new['joint_pos']
                            # print("------------------------")
                            # print(joins_new)
                            # real_arm_joint_pos = np.array(self.env.realUR5.get_j_config())
                            # real_joint_pos = np.concatenate([real_arm_joint_pos, [0.0]])
                            # print(real_joint_pos)
                            # print("------------------------")
                            if observation_new['observation'] is None:
                                print("hidden object")
                                obs_new = obs
                                ag_new = ag
                                joins_new = joins
                        else:
                            # print("prevent collision or success")
                            obs_new = obs
                            ag_new = ag
                            joins_new = joins
                            info = {"is_success": 0.0}
            ...
            if epoch < self.sample_epoch:
                ...
                success_rate, mean_reward = self._eval_agent_real()
            ...
            if MPI.COMM_WORLD.Get_rank() == 0:
                ...
                if epoch % 2 == 0 and self.args.save_data is True:
                    np.save(self.data_path + "/task_hidden_dim{}-interface_dim{}-robot_hidden_dim{}-{}_control-{}_cycles-seed{}-".format(
                        ...
                        self.args.control_type, self.args.n_cycles, self.args.seed) + 'module_sac_push14_norm_sr92', success_rate_record)
                    np.save(self.data_path + "/task_hidden_dim{}-interface_dim{}-robot_hidden_dim{}-{}_control-{}_cycles-seed{}-".format(
                        self.args.task_hidden_dim, self.args.interface_dim, self.args.robot_hidden_dim,
                        self.args.control_type, self.args.n_cycles, self.args.seed) + 'module_sac_push14_norm_rd92', reward_record)

                if epoch % 2 == 0 and self.args.save_model is True:
                    torch.save(
                        ...
                        self.model_path + "/task_hidden_dim{}-interface_dim{}-robot_hidden_dim{}-{}_control-seed{}-".format(
                            self.args.task_hidden_dim, self.args.interface_dim, self.args.robot_hidden_dim,
                            self.args.control_type, self.args.seed) + 'module_sac_push14_norm_full_model92.pt', _use_new_zipfile_serialization=False)
            ...
            if self.args.save_data is True:
                np.save(
                    self.data_path + "/task_hidden_dim{}-interface_dim{}-robot_hidden_dim{}-{}_control-{}_cycles-seed{}-".format(
                        self.args.task_hidden_dim, self.args.interface_dim, self.args.robot_hidden_dim,
                        self.args.control_type, self.args.n_cycles, self.args.seed) + 'module_sac_push14_norm_sr92',
                    success_rate_record)
                np.save(self.data_path + "/task_hidden_dim{}-interface_dim{}-robot_hidden_dim{}-{}_control-{}_cycles-seed{}-".format(
                    self.args.task_hidden_dim, self.args.interface_dim, self.args.robot_hidden_dim,
                    self.args.control_type, self.args.n_cycles, self.args.seed) + 'module_sac_push14_norm_rd92', reward_record)
    ...
    def _eval_agent_real(self):
        total_success_rate = []
        reward_list = []
        self.actor_network.eval()
        for _ in range(10):  # self.args.n_test_rollouts
            per_success_rate = []
            observation = self.env.reset_real2()
            obs = observation['observation']
            g = observation['desired_goal']
            ...
            for _ in range(self.env_params['max_timesteps']):
                ...
                _, _, _, _ = self.env.step(actions, control_method=self.args.control_type)  # step in sim first
                ee_pos = p.getLinkState(self.env.robot.id, self.env.robot.eef_id)[0]  # end effector of the arm without a gripper, a tuple type
                gripper_tip_pos = np.array(ee_pos) - [0, 0, 0.14]  # gripper is approximately 0.14 in length

                # only move the arm if it doesn't hit the cube from top, doesn't hit table, and hasn't succeeded
                if self.env.distance(gripper_tip_pos, ag) > 0.02 and success_flag == 0 and gripper_tip_pos[2] > 0.02:
                    # print("action:")
                    # print(action)
                    observation_new, _, _, info = self.env.step_real2(actions, control_method=self.args.control_type)
                    obs = observation_new['observation']
    ...
    def permutation_visualize_real(self, task_path, robot_path):
        # agent 2 is adversary agent
        total_success_rate = []
        data1 = torch.load(task_path, map_location=torch.device('cpu'))
        data2 = torch.load(robot_path, map_location=torch.device('cpu'))
        ...
        for _ in range(1):
            per_success_rate = []
            observation = self.env.reset_real()
            obs = observation['observation']
            g = observation['desired_goal']
            if obs is None or g is None:
                print("cannot detect object or goal")
                break

            joins = observation['joint_pos']
            for t in range(80):  # for t in range(self.env_params['max_timesteps']):
                with torch.no_grad():
                    task_input_tensor, joins_tensor = self._preproc_inputs(obs, g, joins)
                    ...
                    # pi_1 = torch.zeros_like(pi_2)
                    action = pi.detach().cpu().numpy().squeeze()
                observation_new, _, _, info = self.env.step_real(action, control_method=self.args.control_type)
    ...
    def permutation_visualize_real2(self, task_path, robot_path):
        # This version use a virtual goal position instead of a real one
        total_success_rate = []
        data1 = torch.load(task_path, map_location=torch.device('cpu'))
        data2 = torch.load(robot_path, map_location=torch.device('cpu'))
        self.actor_network.Ptask.load_state_dict(data1[0])
        ...
        for game_num in range(20):
            ...
            for t in range(self.env_params['max_timesteps']):  # for t in range(self.env_params['max_timesteps']):
                with torch.no_grad():
                    if obs is None:
                        print("hidden object")
                        obs = np.array([0, 0, 0])
        ...
        # print(local_success_rate)
        # print('Success rate is: {:.3f}'.format(local_success_rate))
        print('Success rate is: {:.3f}'.format(success_num / 20.0))
    ...
    def permutation_measure(self, task_path, robot_path):
        ...
        np.save('saved_anchors/' + self.args.env_name + '/' + self.args.env_name + '_task_state_2000list92', np.array(task_state_list))
