...
class RobotBase(object):
    ...
    def move_ee(self, action, control_method):
        ...
        if control_method == 'end':
            ...
            ee_orn = p.getEulerFromQuaternion(p.getLinkState(self.id, self.eef_id)[1])
            x, y, z, roll, pitch, yaw = action
            pos = (ee_pos[0] + x, ee_pos[1] + y, ee_pos[2] + z)
            euler_orn = (ee_orn[0] + roll, ee_orn[1] + pitch, ee_orn[2] + yaw)
            ...
            current_joint_obs = self.get_joint_obs()
            # print(current_joint_obs['positions'])
            # print(current_joint_obs['ee_pos'])
            joint_poses = action + current_joint_obs['positions'][:self.arm_num_dofs]
            # print(joint_poses)
        # arm
    ...
    def get_arm_joint_obs(self):
        ...
        for joint_id in self.arm_controllable_joints:
            ...
            positions.append(pos)
            velocities.append(vel)
        return dict(positions=positions, velocities=velocities)
...
# in this class, the robot is lifted to some height to be the same as the real environment
class UR5Robotiq85(RobotBase):
    # height 0.0431
    def __init_robot__(self, pos=(-0.7, -0.1095, 0.0431), ori=(0, 0, 0)):  # self, pos=(-0.48, -0.1095, 0), ori=(0, 0, 0)
        self.eef_id = 7
        self.arm_num_dofs = 6
        self.base_pos = pos
        print(self.base_pos)
        ...
        # self.arm_rest_poses = [0.0, -1.57, 2.2, -2.4,
        #                        -1.571, 0.0]
        self.arm_rest_poses = [0.0, -1.1, 1.3, -1.9,
                               -1.571, 0.0]
        # self.id = p.loadURDF('./urdf/ur5_robotiq_85.urdf', self.base_pos, self.base_ori,
        #                      useFixedBase=True, flags=p.URDF_ENABLE_CACHED_GRAPHICS_SHAPES)
        # self.id = p.loadURDF('robot_env/urdf/ur5_robotiq_85.urdf', self.base_pos, self.base_ori,
        #                      useFixedBase=True, flags=p.URDF_ENABLE_CACHED_GRAPHICS_SHAPES)
        # self.id = p.loadURDF('urdf/ur5_robotiq_85.urdf', self.base_pos, self.base_ori,
        #                      useFixedBase=True, flags=p.URDF_ENABLE_CACHED_GRAPHICS_SHAPES)
        self.id = p.loadURDF('robot_env/urdf/ur5_robotiq_85.urdf', self.base_pos, self.base_ori,
