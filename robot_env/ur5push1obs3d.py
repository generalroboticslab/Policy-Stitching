...
class Ur5Push1Obs3d:
    ...
    def __init__(self, robot, models: Models, camera=None, vis=False) -> None:
        ...
        self.goal_range_high = np.array([0.3 / 2, 0.3 / 2, 0])
        self.obj_range_low = np.array([-0.05 / 2, -0.05 / 2, 0])  # obj_xy_range = 0.05, originally 0.3  easy 0.05
        self.obj_range_high = np.array([0.05 / 2, 0.05 / 2, 0])
        self.action_space_low = -1.0
        self.action_space_high = 1.0
        self.action_shape = 7
        self.task_input_shape = 6  # 3+3
    ...
    def get_obs(self, gripper_open_length):
        ...
        observation = np.array(object_position)
