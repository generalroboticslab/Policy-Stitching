...
# import the class of the real robot
from ur5 import UR5
# import the realsense camera class
from rs_camera_class import rs_camera
...
class Ur5Push1Real:
    ...
    def __init__(self, robot, models: Models, camera=None, vis=False) -> None:
        ...
        # goal2 is for real world training
        self.goal_range_low2 = np.array([-0.10, -0.025, 0])
        self.goal_range_high2 = np.array([0.10, 0.025, 0])
    ...
    def step_real(self, action, control_method='joint'):
        """
        action: (x, y, z, roll, pitch, yaw, gripper_opening_length) for End Effector Position Control
                (a1, a2, a3, a4, a5, a6, a7, gripper_opening_length) for Joint Position Control
        control_method:  'end' for end effector position control
        ...
        print("action:")
        print(action)
        curr_joint_config = np.array(self.realUR5.get_j_config())
        new_config = list(curr_joint_config + action[:-1])
        ...
        # self.robot.move_gripper(action[-1])  # limit the gripper action by 0 - 0.1
        object_position, goal_position = self.rs_camera.detect_pos()
        ...
        if achieved_goal is None or desired_goal is None:
            ...
            info = {"is_success": self.is_success(achieved_goal, desired_goal)}
        return self.get_obs_real(action[-1]), reward, done, info

    def step_real2(self, action, control_method='joint'):
        """
        for real robot fine-tune
        """
        assert control_method in ('joint', 'end')
        action = np.clip(action, self.action_space_low, self.action_space_high)
        # map the [-1, 1] normalized action to the real robot action space
        ...
        self.realUR5.movej(
            use_pos=False,
            params=new_config,
            blocking=True  # originall False
        )
        # # check collision with the object, if collide, don't take this action
        # _, _, _, _ = self.step(action, control_method=control_method)  # step in sim first
        ...
        # # only move the arm if it doesn't hit the cube from top
        # if self.distance(gripper_tip_pos) > 0.05:
        #     self.realUR5.movej(
        #         use_pos=False,
        #         params=new_config,
        #         blocking=False
        ...
        # self.robot.move_gripper(action[-1])  # limit the gripper action by 0 - 0.1
        object_position, _ = self.rs_camera.detect_pos()
        achieved_goal = object_position
        desired_goal = self.get_desired_goal()
        if achieved_goal is None or desired_goal is None:
            reward = -1
            ...
            info = {"is_success": self.is_success(achieved_goal, desired_goal)}
        return self.get_obs_real2(action[-1]), reward, done, info
    ...
    def get_obs_real(self, gripper_open_length):
        # position, rotation of the object
        object_position, goal_position = self.rs_camera.detect_pos()
        # object_position, object_rotation = p.getBasePositionAndOrientation(self._bodies_idx["object"])
        # object_position = np.array(object_position)
        ...
        achieved_goal = object_position
        desired_goal = goal_position
        arm_joint_pos = np.array(self.realUR5.get_j_config())
        joint_pos = np.concatenate([arm_joint_pos, [gripper_open_length]])
        return {
            "observation": observation,
            "joint_pos": joint_pos,
    ...
    def get_obs_real2(self, gripper_open_length):
        # position, rotation of the object
        object_position, _ = self.rs_camera.detect_pos()
        # object_position, object_rotation = p.getBasePositionAndOrientation(self._bodies_idx["object"])
        # object_position = np.array(object_position)
    ...
    def reset_real(self):
        self.realUR5.homej(blocking=True)
        # obj_orig = [0.0, 0.0, self.object_size / 2]
        # obj_noise = np.random.uniform(self.obj_range_low, self.obj_range_high)
        # goal_orig = np.array([0.0, 0.0, self.object_size / 2])  # z offset for the cube center
        # goal_noise = np.random.uniform(self.goal_range_low, self.goal_range_high)
        # p.resetBasePositionAndOrientation(self._bodies_idx["target"], goal_orig+goal_noise, np.array([0.0, 0.0, 0.0, 1.0]))
        # p.resetBasePositionAndOrientation(self._bodies_idx["object"], obj_orig+obj_noise, np.array([0.0, 0.0, 0.0, 1.0]))
        return self.get_obs_real(self.robot.gripper_range[1])

    def reset_real2(self):
        # real world reset, but use a virtual goal position instead of observing it
        self.realUR5.homej(blocking=True)
        # obj_orig = [0.0, 0.0, self.object_size / 2]
        # obj_noise = np.random.uniform(self.obj_range_low, self.obj_range_high)
        goal_orig = np.array([0.0, 0.0, self.object_size / 2])  # z offset for the cube center
        goal_noise = np.random.uniform(self.goal_range_low2, self.goal_range_high2)
        p.resetBasePositionAndOrientation(self._bodies_idx["target"], goal_orig+goal_noise, np.array([0.0, 0.0, 0.0, 1.0]))
        # p.resetBasePositionAndOrientation(self._bodies_idx["object"], obj_orig+obj_noise, np.array([0.0, 0.0, 0.0, 1.0]))
        return self.get_obs_real2(self.robot.gripper_range[1])
