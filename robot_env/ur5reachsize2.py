...
# larger block size
# this env match the pose 3 of the ur5
# The robot is lifted, so I add a cylinder at the base
class Ur5Pos3ReachSize2:
    ...
    def __init__(self, robot, models: Models, camera=None, vis=False) -> None:
        ...
        goal_range = 0.2  # originally 0.3
        self.goal_range_low = np.array([-goal_range / 2, -goal_range / 2, 0])  # original goal_xy_range = 0.3,
    ...
    def step_simulation(self):
        ...
        """
        p.stepSimulation()
        # time.sleep(self.SIMULATION_STEP_DELAY) # seems that don't sleep here won't hurt training
        if self.vis:
    ...
    def step(self, action, control_method='joint'):
        ...
        # # don't wait when training, but this is wrong. Probably hurts the training
        # if self.vis is True:
        #     for _ in range(20):  # Wait for a few steps when rendering, originally 120
        #         self.step_simulation()
        # else:
        #     # self.step_simulation()  # don't wait when training, but this is wrong. Probably hurts the training
        #     for _ in range(20):  # Wait for a few steps when rendering, originally 120
        #         self.step_simulation()
    ...
    def reset(self):
        self.robot.reset()
        # this following three lines are original goal sampling
        # goal_orig = np.array([0.0, 0.0, self.object_size / 2])  # z offset for the cube center
        # goal_noise = np.random.uniform(self.goal_range_low, self.goal_range_high)
        # p.resetBasePositionAndOrientation(self._bodies_idx["target"], goal_orig+goal_noise, np.array([0.0, 0.0, 0.0, 1.0]))

        # this following part is for showing the tip of the gripper
        goal_orig = self.get_achieved_goal()
        goal_noise = np.array([0.0, 0.0, -0.055])
        p.resetBasePositionAndOrientation(self._bodies_idx["target"], goal_orig+goal_noise, np.array([0.0, 0.0, 0.0, 1.0]))
        print("finger_tip:")
        print(goal_orig[2]+goal_noise[2]+self.object_size/2)
